...
[`torvalds/linux`](https://github.com/torvalds/linux)を読む場合は、まずcloneします。GitHub API上のrepository sizeは約6.2GBなので、既定では履歴込みで`$HOME/src/linux`へfull cloneします。
...
```bash
LINUX_REPO_PATH="$HOME/src/linux-mainline" npm run linux:clone
```
